...
def check_or_create_user(email, full_name, membership_plan, payment_status):
    try:
        ...
        if user:
            # Check if a membership record already exists for the user with the same plan
            record_exist = frappe.get_all('MemberShip', 
                filters={"member_name": user[0].name, "membership_plan": membership_plan}, 
                fields=['name', 'payment_status', 'membership_status']
            )
            if record_exist and record_exist[0].payment_status == "Paid" and record_exist[0].membership_status in ["Active", "Free Trial"]:
                return {"status_code": 200, "message": "User already has an active membership"}
            
            # Create membership details if user exists
            membership_doc = frappe.get_doc({
                "doctype": "MemberShip",
                "member_name": user[0].name,  # Use existing user's name
                "membership_plan": membership_plan,
                "start_date": frappe.utils.now_datetime(),
                "payment_status": payment_status,
                "membership_status": "Active" if payment_status == "Paid" else "Inactive",
            })
            membership_doc.insert(ignore_permissions=True)
            return {"status_code": 200, "message": "User exists and Membership created"}
        ...
        new_user = frappe.get_doc({
            "doctype": "User",
            "email": email,
            "first_name": full_name.split()[0],  # Assuming full_name is "First Last"
            "last_name": ' '.join(full_name.split()[1:]),  # To handle possible middle names
            "enabled": 1,
        ...
        # Create membership details after user creation
        membership_doc = frappe.get_doc({
            "doctype": "MemberShip",
            "member_name": new_user.name,
            "membership_plan": membership_plan,
            "start_date": frappe.utils.now_datetime(),
            "payment_status": payment_status,
            "membership_status": "Active" if payment_status == "Paid" else "Pending",
        })
        membership_doc.insert(ignore_permissions=True)

        # Optionally, send a welcome email or notify the user of their new account

        return {"status_code": 201, "message": "User created successfully", "password": password}

    except frappe.exceptions.DuplicateEntryError:
        # Handle the case where the user was created by another request in the meantime
        return {"status_code": 409, "message": "User creation failed due to duplicate entry"}
...
def get_payment_options(doctype, docname, phone, country):
	...
	# controller().validate_transaction_currency(order["currency"])

	payment_details = {
		"amount": cint(details.amount) * 100,
		"title": f"Payment for bill : {details.id}",
		"description": _("Payment for {0} course").format(details["title"]),
		"reference_doctype": "Payment Request",
		"reference_docname": frappe.db.get_single_value("Website Settings", "app_name"),
		...
		"subscription_details": {
			"plan_id": f"plan_{details.id}", # if Required
			"start_date": datetime.date.today(),
			"billing_period": "Month", #(Day, Week, SemiMonth, Month, Year),
			"billing_frequency": 1,
			"customer_notify": 1,
			"upfront_amount": cint(details.amount) * 100
		}
	}
...
def apply_gst(amount, country=None):
	gst_applied = 0
	apply_gst = frappe.db.get_single_value("LMS Settings", "paypal_sandbox")
...
def add_days(date, days):
    """
    Add a specific number of days to the given date.
    
    :param date: The original date (string, datetime, or date object).
    :param days: The number of days to add (integer).
    :return: New date with the added days (as a string in 'YYYY-MM-DD' format).
    """
    # Convert the input date to a datetime.date object
    date = getdate(date)
    days_ = int(days / 86400) if days > 86400 else int(days)
    # Add the specified number of days using timedelta
    new_date = date + timedelta(days=days_)
...
def get_user_membership():
    try:
        ...
        if user_email == "Guest":
            return {"status_code": 403, "message": "User not logged in"}

        # Fetch user's active memberships
        memberships = frappe.get_all("MemberShip",
            filters={
                "member_name": user_email,
                "membership_status": ["in", ["Active", "Free Trial"]]
            },
            fields=["name", "membership_plan", "start_date", "payment_status", "membership_status"]
        )

        # Format the response data
        membership_data = []
        for membership in memberships:
            plan_doc = frappe.get_doc("Membership Plan", membership.membership_plan)

            # Calculate the end date based on the start date and duration from the plan
            start_date = membership.start_date
            duration = int(plan_doc.duration / 86400)  # duration in days
        ...
        return {"status_code": 200, "message": membership_data}

    except Exception as e:
        frappe.log_error(frappe.get_traceback(), 'Get User Membership Error')
        return {"status_code": 500, "message": f"An error occurred: {str(e)}"}
